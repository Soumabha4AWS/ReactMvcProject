...
class List extends Component {
    ...
    componentWillMount() {
        axios.get('http://localhost:8084/SpringBootRestApiMySql/get/all').then((response) => {
    ...
    render() {
        let courses = this.state.courses.map((courses) => {
            return (
                ...
                <tr key={courses.id}> 
                                    <td> 
                                        <Link to={{pathname: '/reactMvcProject/get/editCourses',state: {courses: courses}}}><button type="button" className="btn btn-success btn-sm">Edit</button></Link>
                                    </td>
                                    <td>
                                    <Link to={{pathname: '/reactMvcProject/get/removeCourses', state: {courses: courses}}}><button type="button" className="btn btn-danger btn-sm">Remove</button></Link>
                                    </td>
                                    <th scope="row">
